...
- **Don't use hard-to-distinguish character pairs in names.** (e.g., `l` and `I`, `O` and `0`, etc.). With certain fonts, it is hard to distinguish `firstl` from `firstI`.
...
- **Don't use misleading abbreviations.** E.g., in R, `na.rm` parameter is typically reserved for removing (`rm`) missing values (`NA`). But if you use it to mean "remove (`rm`) non-authorized (`NA`) entries" for your function parameter, it'll be quite misleading.

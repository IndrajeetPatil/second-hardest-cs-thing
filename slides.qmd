...
![](media/cat.png){.absolute width="500" height="350" right="40" top="175"}
:::
...
If a loop is longer than a few lines, use more meaningful loop variable names than `i`, `j`, `k` because you will quickly lose track of what they mean.
...
In the future, it won't be easy either to search for and/or to rename it in the code base because searching for `p` would flag **all** *p*s (gg**p**lot, m**p**g, etc.).
...
knitr::include_graphics("media/osp_naming.png")
```

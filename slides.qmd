...
In the future, it won't be easy either to search for and/or to rename these parameters in the codebase because searching for `a` or `f` would flag **all** *a*s and *f*s (**a**pi, **f**ile, etc.).
...
Names that encode implementation details (e.g., data structure) increase the maintenance cost. When implementation changes, identifier names need to change as well.

. . .

E.g., consider a variable that represents bonuses across company departments:
...
## Choose correct abstraction level for names {.smaller}

Don't fall into the trap of selecting names at a lower level of abstraction.

E.g., if you are writing a function to compute a difference between before and after values, the parameter names should reflect the higher-level concept.
...
```{r}
# bad
diff <- function(value_before, value_after) { ... }
```
...
Note that the good parameter names make it clear that the function is actually of more general use than just temporally separated values.

...
Poor names require heavy commenting:
...
Generic names work *only if* you follow established conventions.
...
Consistent names **reduce cognitive burden** by allowing readers to safely reuse knowledge across contexts.

. . .

These names are inconsistent—*size* means different things in different contexts:
...
**Enable safe assumptions about names across different contexts.**
...
The problem domain names remain meaningful even if you switch from binary search to linear search, from hash tables to arrays, or from SQL to NoSQL databases.

. . .

::: {.callout-tip}

**Name based on the business purpose, not the technical mechanism.**
...
## Naming molds: Templates for consistency {.smaller}

Feitelson's research reveals that developers use **naming molds**—consistent templates for constructing identifiers.

. . .

::: {.callout-note}

## Examples of Different Naming Molds

| **Concept** | **Mold A** | **Mold B** | **Mold C** |
|-------------|------------|------------|------------|
| Maximum user connections | `max_user_connections` | `user_connection_limit` | `peak_user_count` |
| Total file size in bytes | `total_file_size_bytes` | `file_byte_count` | `accumulated_file_bytes` |
| Minimum password length | `min_password_length` | `password_length_floor` | `shortest_password_chars` |
| Current active sessions | `current_active_sessions` | `active_session_count` | `live_session_tally` |
...
**Pick one mold and use it consistently.** Mixed molds in the same codebase force readers to decode different patterns, increasing cognitive load.
...
3. **Construct the name** using a consistent mold

This deliberate process produces names that are more precise and maintainable.
...
Code review provides the mental space needed to evaluate whether names truly capture the intent and abstraction level of your code.
